...
import { Archive, BarChart3, Camera, Home, Sparkles, TrendingUp } from 'lucide-react'
...
export function MobileBottomNav() {
  ...
  const rightTabs = [
    { href: '/sealed', label: 'Coffrets',       icon: Archive },
      ...
      >
        ...
        {isActive && (
          <motion.div
            ...
            className="absolute bottom-0 left-1/2 -translate-x-1/2 w-8 h-0.5 rounded-full bg-pokemon-yellow"
          />
        )}
      </Link>
    )
      ...
      <div className="flex items-center h-16 px-2">
        ...
        <div className="flex flex-col items-center justify-center px-3 -mt-6">
          <Link
            ...
            style={{
              background: 'linear-gradient(135deg, #F59E0B, #FFCB05)',
              boxShadow: isScanActive
                ? '0 2px 12px rgba(255,203,5,0.35)'
                : '0 4px 20px rgba(255,203,5,0.40), 0 2px 8px rgba(245,158,11,0.25)',
            }}
          >
            <Camera className="w-6 h-6 text-[#0A0E18]" />
            {!isScanActive && (
              <motion.div
                className="absolute inset-0 rounded-2xl border-2 border-pokemon-yellow/40"
          ...
          <span className={clsx(
            ...
            isScanActive ? 'text-pokemon-yellow' : 'text-pokemon-yellow/80',
          )}>
            {t.nav.scan}
          </span>
        </div>
