...
interface Props {
  ...
  /** Si true, le modal ne peut pas être fermé sans avoir choisi un pseudo */
  required?: boolean
}

const USERNAME_RE = /^[a-zA-Z0-9_-]{3,20}$/

export function UsernameModal({ open, onClose, initialUsername = '', initialDisplayName = '', required = false }: Props) {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          >
            ...
            <div className="flex items-start justify-between">
              ...
              {!required && (
                <button onClick={onClose} className="text-white/30 hover:text-white/60 transition-colors -mt-1 -mr-1">
                  <X className="w-4 h-4" />
                </button>
              )}
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <input
                ...
                maxLength={30}
                className="w-full px-3 py-2.5 rounded-xl text-sm bg-white/5 border border-white/10 text-white placeholder-white/25 focus:outline-none focus:border-pokemon-yellow/40 transition-colors"
              />
            ...
            {/* Username */}
            <div className="space-y-1.5">
              <label className="text-xs font-semibold text-white/50 uppercase tracking-wide">Pseudo <span className="text-red-400">*</span></label>
              <div className="relative">
                ...
                <input
                  ...
                  maxLength={20}
                  className={clsx(
                    'w-full pl-7 pr-4 py-2.5 rounded-xl text-sm bg-white/5 border text-white placeholder-white/25 focus:outline-none transition-colors font-mono',
            ...
            <button
              ...
              className={clsx(
                'w-full py-3 rounded-xl font-bold text-sm transition-all',
                  ...
                  : 'text-black hover:brightness-110 active:scale-[0.98] disabled:opacity-50'
              )}
              style={done ? {} : { background: 'linear-gradient(135deg, #FFCB05, #F59E0B)' }}
            >
              {done ? '✓ Enregistré !' : loading ? <Loader2 className="w-4 h-4 animate-spin mx-auto" /> : 'Enregistrer'}
            </button>
          </motion.div>
        </div>
      )}
